...
module.exports = function(config, userDB, couchAuthDB, mailer, emitter) {
	...
	this.validateUsername = function(username) {
		...
		if (!username.match(USER_REGEXP)) {
			return BPromise.resolve("اسم المستخدم غير مقبول");
		}
		return userDB.query("auth/username", { key: username }).then(
			function(result) {
				if (result.rows.length === 0) {
					...
					return BPromise.resolve("اسم المستخدم محجوز");
				}
			},
			function(err) {
				throw new Error(err);
	...
	this.validateEmail = function(email) {
		...
		if (!email.match(EMAIL_REGEXP)) {
			return BPromise.resolve("البريد الإلكتروني غير مقبول");
		}
		return userDB.query("auth/email", { key: email }).then(
			function(result) {
				if (result.rows.length === 0) {
					...
					return BPromise.resolve("البريد الإلكتروني مستخدم");
				}
			},
			function(err) {
				throw new Error(err);
	...
	var passwordConstraints = {
		presence: {
			message: "الرمز السري مطلوب"
		},
		length: {
			minimum: 6,
			fullMessages: false,
			message: "يجب أن يكون أكثر من ٦ رموز"
		},
	...
	this.get = function(login) {
		...
		return userDB.query("auth/" + query, { key: login, include_docs: true }).then(function(results) {
				...
				if (results.rows.length > 0) {
					...
					var emailFail = function() {
						return BPromise.reject({
							...
							message: "البريد الإلكتروني مستخدم، قم بتسجل الدخول أولاً من ثم أربط الحساب",
							status: 409
						});
					};
					...
					if (emailUsername) {
						if (!user.email) {
							return BPromise.reject({
								...
								message: "البريد الإلكتروني مطلوب لكن المزود " + provider + " لم يزودنا بالبريد.",
								status: 400
							});
						}
	...
	this.logActivity = function(user_id, action, provider, req, userDoc, saveDoc) {
		...
		return promise.then(function(theUser) {
			...
			if (saveDoc) {
				return userDB.put(userDoc).then(function() {
					return BPromise.resolve(userDoc);
				});
			} else {
				return BPromise.resolve(userDoc);
	...
	this.forgotPassword = function(email, req) {
		...
		return userDB
			...
			.then(function() {
				return mailer.sendEmail("forgotPassword", user.email || user.unverifiedEmail.email, { user: user, req: req, token: token }); // Send user the unhashed token
			})
			.then(function() {
	...
	this.logoutUser = function(user_id, session_id) {
		...
		if (user_id) {
			...
			promise = userDB.query("auth/session", { key: session_id, include_docs: true }).then(function(results) {
				if (!results.rows.length) {
					return BPromise.reject({
						error: "unauthorized",
						status: 401
	...
	function addUserDBs(newUser) {
		...
		var processUserDBs = function(dbList, type) {
			dbList.forEach(function(userDBName) {
				...
				promises.push(
					dbAuth.addUserDB(newUser, userDBName, dbConfig.designDocs, type, dbConfig.permissions, dbConfig.adminRoles, dbConfig.memberRoles).then(function(finalDBName) {
						delete dbConfig.permissions;
						delete dbConfig.adminRoles;
						delete dbConfig.memberRoles;
						delete dbConfig.designDocs;
